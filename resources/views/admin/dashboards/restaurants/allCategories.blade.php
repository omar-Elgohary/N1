...
<section class="container col-lg-12">
    <div class="col-12 d-flex flex-row-reverse p-0">
        <div class="col-6 section-title text-end p-0">
            <h2 class="text-black">أقسام المنيو</h2>
        </div>

        <div class="col-6 text-start">
            <a href="#addCategory" id="coupon" class="btn btn-block btn-bordered px-4 btns" data-bs-toggle="modal">اضافة قسم</a>
        </div>
    </div> <!-- col-12 -->
                ...
                <tbody>
                    @forelse ($categories as $key => $category)
                        <tr>
                            <th>{{ $key+1 }}</th>
                            <td>
                                <a href="{{ route('allSubCategories', $category->id) }}" class="text-warning fw-bold">{{ $category->name }}</a>
                            </td>
                            <td>
                                <a href="#editCategory{{$category->id}}" class="btn bg-white text-success" data-bs-toggle="modal"><i class="fa fa-edit"></i></a>
                                <a href="#deleteCategory{{$category->id}}" class="btn bg-white text-danger" data-bs-toggle="modal"><i class="fa fa-trash"></i></a>
                            </td>
                        </tr>

                        {{-- deleteCategory --}}
                        <div class="modal fade border-0" id="deleteCategory{{$category->id}}" aria-hidden="true" aria-labelledby="deleteCategoryLabel" tabindex="-1" dir="rtl">
                        ...
                        {{-- editCategory --}}
                        <div class="modal fade border-0" id="editCategory{{$category->id}}" aria-hidden="true" aria-labelledby="editCategoryLabel" tabindex="-1" dir="rtl">
                            <div class="modal-dialog modal-dialog-centered">
                                <div class="modal-content">
                                    <div class="btn-x modal-header">
                                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
